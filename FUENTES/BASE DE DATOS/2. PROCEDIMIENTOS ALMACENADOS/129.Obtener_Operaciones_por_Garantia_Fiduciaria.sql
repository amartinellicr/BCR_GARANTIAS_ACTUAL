...
IF OBJECT_ID ('Obtener_Operaciones_por_Garantia_Fiduciaria', 'P') IS NOT NULL
	DROP PROCEDURE dbo.Obtener_Operaciones_por_Garantia_Fiduciaria;
GO

CREATE PROCEDURE [dbo].[Obtener_Operaciones_por_Garantia_Fiduciaria]
	
	@piConsecutivo_Garantia		BIGINT = NULL,
	@pdIdentificacion_Fiador	DECIMAL(12,0),
	@piTipo_Persona_Fiador		SMALLINT = NULL,
	@psCedula_Usuario			VARCHAR (30)
AS
BEGIN

/*****************************************************************************************************************************************************
	<Nombre>Obtener_Operaciones_por_Garantia_Fiduciaria</Nombre>
	<Sistema>BCRGarantías</Sistema>
	<Descripción>
		Procedimiento almacenado que se encarga de obtener las operaciones, contrato y giros de contrato asociados a un fiador detetminardo, además de 
	...
	<Entradas>		
		...
		@pdIdentificacion_Fiador		= Identificación del fiador.
		@piTipo_Persona_Fiador			= Código del tipo de persona del fiador.
...
*****************************************************************************************************************************************************/
	...
	DECLARE	@viTipo_Persona					SMALLINT = NULL,
			@viFecha_Actual_Entera			INT,
			@viCodigo_Tipo_Garantia			SMALLINT = 1,
			@vdIdentificacion_Fiador		DECIMAL(12,0)
	...
	BEGIN
		SELECT	@viTipo_Persona = COALESCE(@piTipo_Persona_Fiador, GGF.cod_tipo_fiador), 
				@vdIdentificacion_Fiador = GGF.Identificacion_Sicc
		FROM	dbo.GAR_GARANTIA_FIDUCIARIA GGF
		WHERE	GGF.cod_garantia_fiduciaria = @piConsecutivo_Garantia
	END
	ELSE
	BEGIN
		SET	@viTipo_Persona = @piTipo_Persona_Fiador 
		SET	@vdIdentificacion_Fiador = @pdIdentificacion_Fiador
	END
	

	/*SE OBTIENEN LOS DATOS DE LA GARANTIA SUMINISTRADA, SEGUN SU TIPO*/
	...
	WHERE	GGF.Identificacion_Sicc = @vdIdentificacion_Fiador
		AND GGF.cod_tipo_fiador = COALESCE(@viTipo_Persona, GGF.cod_tipo_fiador)
