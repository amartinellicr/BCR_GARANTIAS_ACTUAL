...
IF OBJECT_ID ('Insertar_Saldo_Total_Porcentaje_Responsabilidad', 'P') IS NOT NULL
	DROP PROCEDURE dbo.Insertar_Saldo_Total_Porcentaje_Responsabilidad;
GO

CREATE PROCEDURE [dbo].[Insertar_Saldo_Total_Porcentaje_Responsabilidad]
...
/*****************************************************************************************************************************************************
	<Nombre>Insertar_Saldo_Total_Porcentaje_Responsabilidad</Nombre>
	<Sistema>BCRGarantías</Sistema>
	<Descripción>
		Procedimiento almacenado que se encarga de insertar la información referente a los saldos totales y porcentajes de responsabilidad ajustados
	</Descripción>
	<Entradas>		
		...
		@psCedula_Usuario				= Usuario que ingresó el registro
...
*****************************************************************************************************************************************************/
	...
	BEGIN
			SET @psRespuesta = N'<RESPUESTA><CODIGO>1</CODIGO><NIVEL></NIVEL><ESTADO></ESTADO><PROCEDIMIENTO>Insertar_Saldo_Total_Porcentaje_Responsabilidad</PROCEDIMIENTO><LINEA></LINEA>' + 
								'<MENSAJE>El registro ya existe.</MENSAJE><DETALLE>Se produjo un error al insertar los datos del saldo total y porcentaje de responsabilidad. El registro ya existe.</DETALLE></RESPUESTA>'
	...
	BEGIN
			BEGIN TRANSACTION TRA_Act_Registro --Inicio
				BEGIN TRY
					...
					SET		TPR.Saldo_Actual_Ajustado = @pdSaldo_Actual_Ajustado,
							...
							TPR.Usuario_Inserto = @psCedula_Usuario,
							TPR.Fecha_Inserto = GETDATE()
				...
				BEGIN CATCH
					...
					SET @psRespuesta = N'<RESPUESTA><CODIGO>2</CODIGO><NIVEL></NIVEL><ESTADO></ESTADO><PROCEDIMIENTO>Insertar_Saldo_Total_Porcentaje_Responsabilidad</PROCEDIMIENTO><LINEA></LINEA>' + 
										'<MENSAJE>Problema al insertar el registro.</MENSAJE><DETALLE>Se produjo un error al insertar los datos del Saldo Total y Porcentaje de Responsabilidad. El código obtenido es: ' + CONVERT(VARCHAR(1000), @@ERROR) + '</DETALLE></RESPUESTA>'
		...
		SET @psRespuesta = N'<RESPUESTA><CODIGO>0</CODIGO><NIVEL></NIVEL><ESTADO></ESTADO><PROCEDIMIENTO>Insertar_Saldo_Total_Porcentaje_Responsabilidad</PROCEDIMIENTO><LINEA></LINEA>' + 
						'<MENSAJE>La inserción de los datos ha sido satisfactoria. Se activó el registro.</MENSAJE><DETALLE></DETALLE></RESPUESTA>'
	...
	ELSE IF (@viIndicador_Excluido = -1)
	BEGIN
		BEGIN TRANSACTION TRA_Ins_Registro --Inicio
			BEGIN TRY
		
				INSERT	INTO dbo.GAR_SALDOS_TOTALES_PORCENTAJES_RESPONSABILIDAD
				(
				Consecutivo_Operacion,
				Consecutivo_Garantia,
				Codigo_Tipo_Garantia,
				Saldo_Actual_Ajustado,
				Porcentaje_Responsabilidad_Ajustado,
				Porcentaje_Responsabilidad_Calculado,
				Indicador_Ajuste_Saldo_Actual,
				Indicador_Ajuste_Porcentaje,
				Indicador_Excluido,
				Usuario_Inserto,
				Fecha_Inserto
				)				
				...
				VALUES
				(
				@piConsecutivo_Operacion,
				@piConsecutivo_Garantia,
				@piCodigo_Tipo_Garantia,
				@pdSaldo_Actual_Ajustado,
				@pdPorcentaje_Responsabilidad_Ajustado,
				@pdPorcentaje_Responsabilidad_Calculado,
				0,
				0,
				0,
				@psCedula_Usuario,
			...
			BEGIN CATCH
				IF (@@TRANCOUNT > 0)
					ROLLBACK TRANSACTION TRA_Ins_Registro
		...
		IF (@@TRANCOUNT > 0)
			COMMIT TRANSACTION TRA_Ins_Registro --Fin
