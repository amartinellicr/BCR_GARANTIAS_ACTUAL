...
******************************************************************/
	...
	DECLARE @TMP_GARANTIAS_REALES_OPERACIONES TABLE (	cod_bien					VARCHAR (25)	COLLATE DATABASE_DEFAULT,
	...
	BEGIN
		SELECT	DISTINCT CASE GRO.cod_tipo_garantia_real  
							WHEN 1 THEN '[Hipoteca] ' + COALESCE(GRO.Garantia_Real,'') 
							WHEN 2 THEN '[Cédula Hipotecaria] ' + COALESCE(GRO.Garantia_Real,'')
							WHEN 3 THEN '[Prenda] ' + COALESCE(GRO.Garantia_Real,'') 
						 END AS garantia
