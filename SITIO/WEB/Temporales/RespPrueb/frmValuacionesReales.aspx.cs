...
namespace BCRGARANTIAS.Forms
{
    public partial class frmValuacionesReales : BCR.Web.SystemFramework.PaginaPersistente
    {
        ...
        private int aplicarValidaciones = 0;
                    ...
                    if (Gestor.IsInRol(Global.UsuarioSistema, int.Parse(Application["MNU_VALUACION_REAL"].ToString())))
                    {
                        ...
                        if (datosGenerales.Length > 0)
                        {
                            ...
                            if (datosGen.Length == 11)
                            {
                                ...
                                listaDatosGenerales.Add(clsGarantiaReal._fechaConstitucion, datosGen[9]);
                                listaDatosGenerales.Add(clsGarantiaReal._fechaPresentacion, datosGen[10]);

                                long fechaConsti = long.Parse(datosGen[9]);
                                long fechaPresent = long.Parse(datosGen[10]);

                                FechaConstitucion = new DateTime(fechaConsti);
                                FechaPresentacion = new DateTime(fechaPresent);
        ...
        {
            int cantidadAvaluos = ((gdvValuacionesReales.Rows.Count <= 0) ? 0 : gdvValuacionesReales.Rows.Count);

            Response.Redirect("frmGarantiasReales.aspx?CAv=" + cantidadAvaluos.ToString(), true);
        }
                ...
                switch (e.CommandName)
                {
                    case ("SelectedValuacionReal"):
                        ...
                        try
                        {
                            ...
                            fechaPresentacionBD = ((gdvValuacionesReales.SelectedDataKey[14].ToString() != null) && (DateTime.TryParse(gdvValuacionesReales.SelectedDataKey[14].ToString(), out fechaPresentacion)) ? fechaPresentacion : DateTime.MinValue);
                            FechaPresentacion = ((FechaPresentacion >= fechaPresentacionBD) ? FechaPresentacion : fechaPresentacionBD);
                            ...
                            aplicarValidaciones = int.Parse((Request.QueryString["ApVal"] != null) ? Request.QueryString["ApVal"].ToString() : "0");

                            if (tipoInconsistencia != 0)
                            {
                                ValidarTipoInconsistencia(tipoInconsistencia);
                            }
                            else if (aplicarValidaciones == 1)
                            {
                                AplicarValidaciones();
                            }
                        }
            ...
            try
            {
                if (listaDatosGenerales.Count == 0)
                {
                    ...
                    if (dsDatos.Tables["Garantia"].Rows.Count > 0)
                    {
                        ...
                        else if (int.Parse(dsDatos.Tables["Garantia"].Rows[0][2].ToString()) == int.Parse(Application["CEDULAS_HIPOTECARIAS"].ToString()))
                        {
                            txtPartido.Text = dsDatos.Tables["Garantia"].Rows[0][3].ToString();
                            txtNumFinca.Text = dsDatos.Tables["Garantia"].Rows[0][4].ToString();
                            txtGrado.Text = ((dsDatos.Tables["Garantia"].Rows[0][5].ToString().CompareTo("-1") != 0) ? dsDatos.Tables["Garantia"].Rows[0][5].ToString() : string.Empty);
                            txtCedulaHipotecaria.Text = ((dsDatos.Tables["Garantia"].Rows[0][6].ToString().CompareTo("-1") != 0) ? dsDatos.Tables["Garantia"].Rows[0][6].ToString() : string.Empty);
                            lblPartido.Text = "Partido:";
                            lblFinca.Text = "Número Finca:";
                ...
                else
                {
                    ...
                    else if (int.Parse(listaDatosGenerales[clsGarantiaReal._codTipoGarantiaReal]) == int.Parse(Application["CEDULAS_HIPOTECARIAS"].ToString()))
                    {
                        txtPartido.Text = listaDatosGenerales[clsGarantiaReal._codPartido];
                        txtNumFinca.Text = listaDatosGenerales[clsGarantiaReal._numeroFinca];
                        txtGrado.Text = ((listaDatosGenerales[clsGarantiaReal._codGrado].CompareTo("-1") != 0) ? listaDatosGenerales[clsGarantiaReal._codGrado] : string.Empty);
                        txtCedulaHipotecaria.Text = ((listaDatosGenerales[clsGarantiaReal._cedulaHipotecaria].CompareTo("-1") != 0) ? listaDatosGenerales[clsGarantiaReal._cedulaHipotecaria] : string.Empty);
                        lblPartido.Text = "Partido:";
                        lblFinca.Text = "Número Finca:";
                        lblGrado.Visible = true;
                        lblCedula.Visible = true;
        ...
        /// <summary>
        /// Se realiza la validación del tipo de inconsistencia, mostrando el mensaje de error correspondiente
        /// </summary>
        /// <param name="codigoInconsistencia">Indicador del tipo de inconsistencia a mostrar</param>
            ...
            if (MostrarMensaje)
            {
                ...
                if (codigoInconsistencia == 3)
                {
                    ...
                    if (txtFechaConstruccion.Text.Trim().Length >= 0) 
                    {
                        ...
                        if (fechaConstruccion != DateTime.MinValue)
                        {
                            if ((fechaConstruccion <= FechaConstitucion) && (fechaConstruccion < FechaPresentacion))
                            {
                                mostrarError = false;
                            }
                        }
        ...
        /// <summary>
        /// Se aplican las validaciones en caso de quese haya dado un cambio en el tipo de bien
        /// </summary>
